...
# desc means sorting columns in a data frame in descending order
...
# frequency of fear-related words in tidy_tweets
...
create_subset <- function(corpus, username) {
  subset_corpus <- corpus_subset(corpus, username == !!username)
  return(subset_corpus)
}

analyze_sentiment <- function(corpus, sentiment_dict) {
  tidy_tweets <- tidy(corpus)
  
  sentiment_data <- tidy_tweets %>%
    inner_join(get_sentiments(sentiment_dict)) %>%
    count(username, sentiment) %>%
    spread(sentiment, n, fill = 0) %>%
    mutate(sentiment = positive - negative)
  
  ggplot(sentiment_data, aes(username, sentiment)) +
    geom_point(alpha = 0.5) +
    geom_smooth(method = loess, alpha = 0.25) +
    ylab(paste(sentiment_dict, "sentiment"))
}

tidy_tweets %>%
  inner_join(get_sentiments("bing")) %>%
  count(username, index = order %/% 1000, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative) %>%
...
newspaper_sources <- unique(docvars(toks_news)$username)

sentiment_dict <- "data_dictionary_LSD2015"
sentiment_results <- list()

for (source in newspaper_sources) {
  subset_corpus <- create_subset(toks_news, source)
  sentiment_plot <- analyze_sentiment(subset_corpus, sentiment_dict)
  sentiment_results[[source]] <- sentiment_plot
}

tidy_tweets %>%
  mutate(obs=1) %>%
  filter(grepl(paste0(data_dictionary_LSD2015, collapse = "|"), word, ignore.case = T)) %>%
  group_by(username) %>%
  summarise(sum_mwords = sum(obs)) %>%
  full_join(totals, word, by="username") %>%
  mutate(sum_mwords= ifelse(is.na(sum_mwords), 0, sum_mwords),
         pctmwords = sum_mwords/sum_words) %>%
  ggplot(aes(username, pctmwords)) +
  geom_point(alpha=0.5) +
  geom_smooth(method= loess, alpha=0.25) +
  xlab("Newspaper") + ylab("% sentiment")
